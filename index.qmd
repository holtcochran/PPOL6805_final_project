...
dc_traffic_all_years_hist <- dc_traffic_all_years %>%
  filter(!is.na(AADT) & AADT != 0 & AADT >= 50000) %>%  # Exclude NA, 0, and values less than 20,000
  mutate(AADT_bin = cut(
    AADT,
    breaks = seq(0, max(AADT, na.rm = TRUE), by = 50000),  # Bin size of 50,000
    include.lowest = TRUE,
    right = FALSE,
    labels = paste0(
      round(seq(0, max(AADT, na.rm = TRUE) - 50000, by = 50000) / 1000, 1), "k-",
      round(seq(50000, max(AADT, na.rm = TRUE), by = 50000) / 1000, 1), "k"
    )
  ))

# Plot AADT distribution by year with updated bin labels
AADT_Histogram <- ggplot(dc_traffic_all_years_hist, aes(x = factor(year), fill = AADT_bin)) +
...
I then a spatial autoregressive (SAR) model to test the statistical significance of the changes in dispersion of the traffic in Washingon, DC. 
